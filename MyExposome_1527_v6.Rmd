...
---
<!--
### THIS IS THE OUTPUT SECTION on 10/15/2024 @ 11:18am works fine...
...
<!-- THE CSS below sets the TOOLTIPS i used to be a bigger font and changes background color and border... to be more readable    

# THIS DID NOT WORK but... so what for now..
#bs.tooltip
#a.fn.tooltip
...
.tooltip:hover::after {
    ...
    position: absolute;
    background-color: #f0f0f0; /* Tooltip background */
...
OR DID THIS 
ALSO GOOD:  THIS IS MY CURRENT FAVORITE
  rmdformats::readthedown:
    fig_height: 14
    fig_width: 10
    self_contained: true
...
ALSO GOOD I THINK:
  ...
  Other things to fix:
    we use "week worn or time adjusted or weight adjusted as flags for printing MESSAGES but mostly we 
    do NOT use those same flags for deciding FIXUP... we set fixup flags based on what test done 
      THIS IS INCONSISTENT and easy to break and should be fixed
...
`r if(nrow(testResults.big[testResults.big[, "Flag"] == "Y", ]) >0){                             paste("**IMPORTANT NOTE:     ",length(unique(testResults.big[testResults.big[, "Flag"] == "Y", ]$ParameterName)),                             "of the tested compounds impacting ",                             length(unique(testResults.big[testResults.big[, "Flag"] == "Y", ]$SampleNumber)),                             "wristbands could not be fully quantified.**")                             }` `r if(nrow(testResults.big[testResults.big[, "Flag"] == "Y", ]) >0){                             paste("\n Due instrument challenges some chemicals were present but could not be quantified.",                               "For reporting below, where numbers are needed, these specific results are counted as though",                               "they detected zero because we don't have an actual number to use in this report",                               "but we can report that these compounds were definitely detected on these wristbands")                             }`
