...
<script>
  function openTab(tabId) {
    // Locate the tab header
    const tabHeader = document.querySelector(`a[href="#${tabId}"][role="tab"]`);
    if (tabHeader) {
      // Activate the correct tab
      const tabLi = tabHeader.closest("li[role='presentation']");
      const tabList = tabHeader.closest("ul[role='tablist']");

      if (tabList) {
        // Remove "active" class from all tabs
        const allTabs = tabList.querySelectorAll("li[role='presentation']");
        ...
        allTabContents.forEach((content) => {
          content.classList.remove("show", "active"); // Clear "show" and "active" from all content
        });

        const tabContent = document.getElementById(tabId);
        if (tabContent) {
          tabContent.classList.add("show", "active"); // Activate the selected tab content
        }

        // Scroll to the nearest H2 heading above the tab list
        setTimeout(() => {
          // Find the nearest H2 above the tab list
          ...
          while (currentElement) {
            ...
            currentElement = currentElement.previousElementSibling; // Traverse upwards in the DOM
          }
          ...
          if (headingToScrollTo) {
            ...
            // Fallback: Scroll to the tab content with slight offset
